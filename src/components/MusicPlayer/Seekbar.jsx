...
const Seekbar = ({ value, min, max, onInput, setSeekTime, appTime }) => {
  ...
  return (
    <div className="hidden sm:flex flex-row items-center">
      ...
      <p className="text-white w-[2rem]">{value === 0 ? '0:00' : getTime(value)}</p>
      <input
        type="range"
        step="any"
        value={value}
        ...
        max={max}
        onInput={onInput}
        className="range md:block w-24 md:w-56 2xl:w-96 mx-4 2xl:mx-6 "
      />
      <p className="text-white w-[2rem]">{max === 0 ? '0:00' : getTime(max)}</p>
